            ...
            if os.path.exists(filename):
                ...
                for j in range(Y - int(FilterSize(c)) // 2 * 2): #卷積陣列計算
                    for i in range(X - int(FilterSize(c)) // 2 * 2):
                        ...
                        a[j + int(FilterSize(c)) // 2][i + int(FilterSize(c)) // 2] = sum(sum(W))
                        n += 1
                    n = 0
                    m += 1
                ImgLOG = np.array(a)
                    
                AfterImage = [] #卷積計算後圖片每一畫素值轉成List
                for y in range(0, Y):
                    img_gray1 = []
                    for x in range(0, X):
                        img_gray1.append(ImgLOG[y, x])
                    AfterImage.append(img_gray1)        
#-----------------------------------------------H Band 1D Filter 影像卷積convolution處理
                ...
                for j in range(Y): #卷積陣列計算
                    for i in range(X - int(FilterSize(cb)) // 2 * 2):
                        ...
                        b[j][i + int(FilterSize(cb)) // 2] = sum(sum(W))
                        n += 1
                    n = 0        
                ImgLOGB = np.array(b)
                    
                AfterImageB = [] #卷積計算後圖片每一畫素值轉成List
                for y in range(0, Y):
                    img_gray2 = []
                    for x in range(0, X):
                        img_gray2.append(ImgLOGB[y, x])
                    AfterImageB.append(img_gray2)
                
#----------------------------------------------V Band 1D Filter 影像卷積convolution處理
                bv = [[0] * X for _ in range(Y)]
                W = 0
                m = 0
                for j in range(Y - int(FilterSize(cv)) // 2 * 2):
                    for i in range(X):
                        W = img[j:int(FilterSize(cv)) + m, i] * LOGVFilter_array[0:int(FilterSize(cv)), 0]
                        bv[j + int(FilterSize(cv)) // 2][i] = sum(W)
                    m += 1
                ImgLOGV = np.array(bv)

                AfterImageV = []#卷積計算後圖片每一畫素值轉成List
                for y in range(0, Y):
                    img_gray3 = []
                    for x in range(0, X):
                        img_gray3.append(ImgLOGV[y, x])
                    AfterImageV.append(img_gray3)
